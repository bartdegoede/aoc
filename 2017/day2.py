...
def parse_spreadsheet2(data):
    total = 0
    for row in data.split('\n'):
        values = [int(v) for v in row.split()]
        for pair in itertools.combinations(values, 2):
            sorted_pair = sorted(pair, reverse=True)
            if sorted_pair[0] % sorted_pair[1] == 0:
                total += sorted_pair[0] / sorted_pair[1]
                break
    print total

if __name__ == '__main__':
    data = """515 912 619 2043    96  93  2242    1385    2110    860 2255    621 1480    118 1230    99
